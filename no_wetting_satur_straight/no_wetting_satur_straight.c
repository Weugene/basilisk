...
double cylindersOy(double x, double y, double z, double Ox, int Ncyl, double R) {
    int icyl;
...
double geometry(double x, double y, double z, double R) {
    const int Nlayer=4;
    int icyl;
    double dist = 0.5*L0/Nlayer;
    double geom = cylindersOy(x, y, z, 0.0, Ncyl, R);

    for (icyl=1; icyl<Nlayer; icyl++){
        geom = max(cylindersOy(x, y, z, icyl*dist, Ncyl, R), geom); //straight
//        geom = max(cylindersOy(x, y+dist/2.*(icyl%2), z, icyl*dist, Ncyl + icyl%2), geom); //staggered
    }
...
//    }
  ...
  which are very long. */
    ...
    TOLERANCE = 1e-4;
    /**
    We need to tell the code that both `fiber` and `f` are volume
    fraction fields. */

    for (scalar s in {fiber,f}) {
...
//        fclose (fp);
        ...
        do {
            iteration++;
            fraction(fiber, geometry (x, y, z, R));
            //fraction(fiber_in, geometry (x, y, z, R - 2*L0*pow(2,-LEVEL)));
            fraction(f, bubbles (x, y, z));
        }while (adapt_wavelet({fiber, f}, (double []){feps, feps},
                              maxlevel = MAXLEVEL, minlevel=5).nf != 0 && iteration <= 10);

        foreach() {
            f[] -= fiber[];// perfect fixed wetting drops instead of solid cylinders
            u.x[] = 1.;
        }
        boundary ({f, u.x, u.y}); //?
#if dimension == 2
