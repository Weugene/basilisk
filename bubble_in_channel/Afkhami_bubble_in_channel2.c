...
void bubbles (scalar f)
{
    ...
    #else
    fraction (f, sq(x/Radius_b2) + sq(y/Radius_b) - sq(1) );
    #endif
}
...
event logfile (i +=100)
{
	...
	fprintf (ferr, "%d %d %g %g %g %g %g %g %g %g %g %g %g \n", maxlevel, i, t, dt, avggas, velgx, velgy, velamean, velgx/(sq(Radius_b)*B), velgx/U0, (velgx/U0 - 1), log(fabs(velgx/U0 - 1)),  xcg);
}
...
#define ADAPT_EPS_SCALARS {1e-3, 1e-3, 0.05*U0, 0.05*U0}
event adapt (i++){
