...
event velocity (i++) {
  foreach()
    foreach_dimension()
      u.x[] = (1. - tangaroa[])*u.x[];//velocity = 0 in the solid
  boundary ((scalar *){u});
}
...
/*event movie (i += 1; t <= 10)
{
  ...
  draw_vof ("tangaroa", fc = {0.5,0.5,0.5});//tangaroa is grey
  draw_vof ("f", color = "Z", min = -0.1, max = 0.1, linear = true);
  scalar l2[];
  lambda2 (u, l2);
  isosurface ("l2", -100);
...
event adapt (i++) {
  ...
  adapt_wavelet ({f,tangaroa,u},
		 (double[]){0.01,0.01,uemax,uemax,uemax}, MAXLEVEL, 5);// maxlevel=MAXLEVEL, minlevel=5
}
