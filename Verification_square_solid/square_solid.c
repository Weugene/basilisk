...
double U0=1, rhol=1, sig=0.0005, Lchar=1, mul=1, Lb=0.3, a_solid=0.1;
double Rrho=1, Rmu=1;
double RE, CA, Rrad;
double xs0 = -0.3, Rb = 0.0625;
...
int main(int argc, char * argv[])
{
    ...
    fprintf(ferr, "maxlevel=%d tol=%g NITERMAX=%d\n"
                  "RE=%g CA=%g Rb/a_solid=%g rho1/rho2=%g mu1/mu2=%g\n"
...
void soild_fs(scalar fs, double t){
    ...
    foreach_vertex() {
        ...
        for (int xi=-L0; xi <=L0; xi +=L0) {
            double x1 = x00 + xi;
            phi[] = intersection(phi[], pow(x - x1,10) + pow(y,10) - pow(0.5*a_solid, 10));//min(a,b)
//            phi[] = intersection(phi[], (fabs(x - x1) > 0.5*a_solid || fabs(y) > 0.5*a_solid) ? 1 : -1);//min(a,b)
        }
        phi[] = -phi[];
    }
    boundary ({phi});
...
event vtk_file (t += 0.01){
    ...
    output_vtu_MPI( (scalar *) {fs, f, omega, p, l, divu, rho}, (vector *) {u, g, dbp, total_rhs, a, mu_low, mapped_data_lower, mapped_data_upper}, subname, 1 );
}
