...
#define T_BC ((x <= 0.0) ? T_solid : (fabs(x) <= 1.0) ? \
                                           (0.5*(Tin + T_solid) + 0.5*(T_solid - Tin)*cos(pi*x)) : \
                                           Tin)
...
// Wall at x=0
double geometry_wall(double x, double y, double z){
    return -x;
}

void solid_func_band(scalar fs){
    double mindelta = L0 / (1 << maxlevel);
    vertex scalar phi[];
    foreach_vertex() {
        double phi1 = geometry_wall(x, y, z);
        double phi2 = geometry_wall(x, y, z) - Nsb * mindelta;
        phi[] = min(phi1, -phi2);
    }
    boundary ((scalar *){phi});
    fractions (phi, fs);
...
FILE *fp;
    ...
    sprintf(name_vtu, "wall_polymerization_basilisk_%s.csv", prefix);
