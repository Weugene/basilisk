...
//	  "meshlabserver -i MY_FILE.ply -o distance.stl)");
//  system ("test -f distance.stl || "
//        "(meshlabserver -i MY_FILE.ply -o distance.stl)");
  ...
  scalar d[];
  distance (d, p);
  while (adapt_wavelet ({d}, (double[]){5e-1*L0}, 10).nf);
...
/**
Note that the "tail" of the MY_FILE is an artefact due to an
inconsistency in the surface mesh, which is self-intersecting near
this point.

![Isosurface of the distance function coloured with level of refinement.](distance/MY_FILE.png)
