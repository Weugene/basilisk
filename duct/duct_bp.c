...
//  fs.refine = fs.prolongation = fraction_refine; //cs is a volume fraction field
...
event vtk_file (t += 1){
  char subname[80]; sprintf(subname, "duct_bp_dt=1e-3");

  output_vtu_MPI( subname, t + dt, (scalar *) {p, fs},
  (vector *) {u} );
}
