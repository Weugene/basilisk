...
parser.add_argument(
    "-contourList",
    type=int,
    help="Provide the list of contour (can be overwritten further), please. Has higher priority",
    nargs="+",
    default=[0, 0, 0],
)
parser.add_argument(
    "-contourStartEndStep",
    type=int,
    help="Provide the start, end and step of output (can be overwritten further), please",
    nargs="+",
    default=[0, 0, 0],
)
parser.add_argument(
    "-lDomain",
    type=float,
    help="Provide the domain size, please",
    nargs="?",
    default=20.0,
)
parser.add_argument(
...
lDomain = max([abs(bounds[1] - bounds[0]), abs(bounds[3] - bounds[2]), abs(bounds[5] - bounds[4])])
centerF = [(bounds[0] + bounds[1]) / 2, (bounds[2] + bounds[3]) / 2, (bounds[4] + bounds[5]) / 2]
print("lDomain=", lDomain, " centerFirst=", centerF)
...
if not noVideo:
    SaveAnimation(
        fn, renderView1, ImageResolution=[2048, 512], FrameRate=frameRate, FrameWindow=frameWindow,
    )
...
# init the 'PiecewiseFunction' selected for 'OSPRayScaleFunction'
contour3Display.OSPRayScaleFunction.Points = [
    0.001414213562373095,
    0.0,
    0.5,
    0.0,
...
# init the 'PiecewiseFunction' selected for 'OSPRayScaleFunction'
line1Display.OSPRayScaleFunction.Points = [
    0.001414213562373095,
    0.0,
    0.5,
    0.0,
...
# init the 'PiecewiseFunction' selected for 'OSPRayScaleFunction'
line2Display.OSPRayScaleFunction.Points = [
    0.001414213562373095,
    0.0,
    0.5,
    0.0,
