...
event velocity (i++) {
    foreach()
		...
		if(isnan(u.x[])) {printf("u.x[] is nan at step=%d and time t=%g", i, t); return 888;}
    	}
    boundary ((scalar *){u});
}
...
event vtk_file (i += 100)
{
    ...
    output_vtk ({f, fiber, u.x, u.y, rhov, p}, 512, fplist, false );
...
event adapt (i++) {
    ...
    adapt_wavelet ({f,fiber,u},
                   (double[]){0.01,0.01,uemax, uemax, uemax}, maxlevel = MAXLEVEL, minlevel = 5);// maxlevel=MAXLEVEL, minlevel=5
}
