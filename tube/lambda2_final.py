...
parser.add_argument(
    "-vidName",
    type=str,
    help="Provide the name for the outputed video, please",
    nargs="?",
    default="lambda2",
)
parser.add_argument(
...
parser.add_argument(
    "-filenames",
    type=str,
    help="Provide the name of the outputing paraview files, please",
    nargs="?",
    default="*.pvd",
)
parser.add_argument(
    "-frameRate",
    type=int,
    help="Provide the frame rate for the video, please",
    nargs="?",
    default=10,
)
parser.add_argument(
    "-frameWindow",
    type=int,
    help="Provide the frame window (can be overwritten further), please",
    nargs="+",
    default=[0, 0],
)
parser.add_argument(
...
parser.add_argument(
    "-nslices",
    type=int,
    help="Provide the number of slices",
    nargs="?",
    default=10,
)
...
parser.add_argument(
    "-viewSize",
    type=int,
    help="Provide the view size of the output pictures, please",
    nargs="+",
    default=[3108, 1168],
)
parser.add_argument(
    "-noVideo",
    type=bool,
    help="Provide the no video Mode",
    nargs="?",
    default=False,
)
parser.add_argument(
    "-noPic",
    type=bool,
    help="Provide the no Picture Mode",
    nargs="?",
    default=False,
)
parser.add_argument(
    "-noData",
    type=bool,
    help="Provide the no Data Exporting Mode",
    nargs="?",
    default=False,
)
parser.add_argument(
    "-noLambda2",
    type=bool,
    help="Provide the no Lambda2 Mode",
    nargs="?",
    default=False,
)
...
if vtk_from_pvpython:
    if filenames[-5::] == ".pvtu":
        ...
        filenames = []
        for i in numbers:
            filenames.append(f"{path}/{file}{i:04d}.pvtu")
        print(filenames)
        fn = path + "/" + filenames
...
if not noPic:
    for i in timeList:
        ...
        #         print('u_mean:',u_mean)
        s = f"time: {timesteps[i]} i: {i} coarse_x_mean: {x_mean}  "
        print(s)
        if vtk_from_pvpython:
            my_stats.write(s)
        ...
        Show(extractSurface1, renderView1, "GeometryRepresentation")
        ...
        s = f"refined_x_mean: {x_mean} refined_u_mean: {u_mean} len_min: {len_min} len_max: {len_max} len: {length} len_bub: {len_bub} "
        print(s)
        if vtk_from_pvpython:
            my_stats.write(s)
        ...
        isoVolume2.ThresholdRange = [-2.0, -1.0]
        ...
        programmableFilter1.Script = """regionArray = inputs[0].CellData['RegionId']
        ...
        rgX, rgY, rgZ = (
            ...
            np.max([abs(range2[0]), abs(range2[1])]),
        )
        ...
        #         threshold1Display.AmbientColor = [0.0392156862745098, 0.00784313725490196, 1.0]
        #         threshold1Display.ColorArrayName = ['POINTS', '']
        #         threshold1Display.DiffuseColor = [0.0392156862745098, 0.00784313725490196, 1.0]
        #         threshold1Display.Opacity = 0.8
        #         threshold1Display.Specular = 1.0
        #         threshold1Display.SpecularPower = 1.0
        #         threshold1Display.Ambient = 0.21
        ...
        #
        #         # init the 'PiecewiseFunction' selected for 'ScaleTransferFunction'
        #         threshold1Display.ScaleTransferFunction.Points = [0.5, 0.0, 0.5, 0.0, 0.5001220703125, 1.0, 0.5, 0.0]
        #
        #         # init the 'PiecewiseFunction' selected for 'OpacityTransferFunction'
        ...
        selection.QueryString = (
            "(pointIsNear([(" + str(lDomain) + ",0,0),], " + str(lDomain) + ", inputs))"
        )
        ...
        for k, sl in enumerate(np.linspace(bounds[0] + 0.1, bounds[1] - 0.1, nslices)):
            ...
            text1 = Text()
            text1.Text = (
                "x=" + str(round(sl, 2)) + " l/l_b=" + str(round((sl - bounds[0]) / len_bub, 2))
            )
            ...
            extractSurface1Display.Opacity = 1
            extractSurface1Display.Specular = 0.8
            extractSurface1Display.SpecularPower = 100.0
            extractSurface1Display.Ambient = 0.1
            extractSurface1Display.OSPRayScaleArray = "f"
            extractSurface1Display.OSPRayScaleFunction = "PiecewiseFunction"
            extractSurface1Display.SelectOrientationVectors = "None"
            extractSurface1Display.ScaleFactor = 0.3379646740552927
            ...
            extractSurface1Display.GaussianRadius = 0.016898233702764633
            extractSurface1Display.SetScaleArray = ["POINTS", "f"]
            extractSurface1Display.ScaleTransferFunction = "PiecewiseFunction"
            extractSurface1Display.OpacityArray = ["POINTS", "f"]
        ...
        for k, sl in enumerate(np.linspace(bounds[0] + 0.1, bounds[1] - 0.1, nslices)):
            ...
            extractSurface1Display.ColorArrayName = ["POINTS", "absOmega"]
            extractSurface1Display.LookupTable = omegaLUT
            extractSurface1Display.Opacity = 1
            extractSurface1Display.Specular = 0.8
            extractSurface1Display.SpecularPower = 100.0
            extractSurface1Display.Ambient = 0.1
