...
def calc_thickness(x, y, x_peak, x_mean, prefix):
    ...
    print(
        ...
        "delta_avg_std=",
        delta_avg_std,
        "NOTE: here avg is calsculated differently",
    )
...
def pca_sort(x, y, M=3, window=100, is_cycle=False, dcut=0.05):
    ...
    cut_x1, cut_x2 = xmin + 0.08 * (xmax - xmin), xmin + 0.7 * (xmax - xmin)

    inds = [
        (x < cut_x1 + dcut * (xmax - xmin)),
...
def sort_and_smooth(
    ...
    x = np.concatenate(
        [
            xy0[0] + length_x_clip_ends * np.linspace(-0.01, 0, wdw),
            x,
            xyN[0] + length_x_clip_ends * np.linspace(0, 0.01, wdw),
        ],
    )
    ...
    # cut_x1, cut_x2 = xmin + cut_1*(xmax-xmin), xmin + cut_2*(xmax-xmin)
    cut_x1, cut_x2 = x_peak - dl1, x_peak + dl2
    ...
    print(f"cut_x1={cut_x1} +/- {dcut*(xmax-xmin)}, cut_x2={cut_x2} +/- {dcut*(xmax-xmin)}")
    ccenters = np.zeros((M, 2))
    color = plt.cm.rainbow(np.linspace(0, 1, M))
    for i, c in zip(range(M), color):
        x_, y_ = np.asarray(x[inds[i]]), np.asarray(y[inds[i]])
    ...
    for i, c in zip(range(M), color):
        ...
        print(
            f"Cluster {i} contains {X[i].size} points within \
        ...
        if chebushev:
            ...
            x_ = np.linspace(x_[0], x_[-1], Ninterp // M)
            y_ = p(x_)
            print(f"Chebushev is done xminmax={min(x_)} {max(x_)}")
...
def find_smooth_curve_and_bounds(
    x,
    y,
    x_mean,
    alpha=0.05,
    ...
    XX, YY, length_x_clip_ends, xy0, xyN, xmin, xmax = sort_and_smooth(
        x,
        y,
        x_mean,
        x_peak,
    ...
    delta_min_smooth, delta_max_smooth, delta_avg_fil, delta_avg_std_fil = calc_thickness(
        XX, YY, x_peak, x_mean, "filtered",
    )
    delta_min_lw, delta_max_lw, delta_avg_lw, delta_avg_std_lw = calc_thickness(
        lower_hull[:, 0], lower_hull[:, 1], x_peak, x_mean, "lower_hull",
    )
    delta_min_up, delta_max_up, delta_avg_up, delta_avg_std_up = calc_thickness(
        upper_hull[:, 0], upper_hull[:, 1], x_peak, x_mean, "upper_hull",
    )
    delta_min, delta_max, delta_avg, delta_avg_std = calc_thickness(
        x, y, x_peak, x_mean, "sliced_x_y",
    )

    lh_fil_x, lh_fil_y, lh_length, lh_xy0, lh_xyN, lh_xmin, lh_xmax = sort_and_smooth(
        lower_hull[:, 0],
        lower_hull[:, 1],
        x_mean,
        x_peak,
    ...
    uh_fil_x, uh_fil_y, uh_length, uh_xy0, uh_xyN, uh_xmin, uh_xmax = sort_and_smooth(
        upper_hull[:, 0],
        upper_hull[:, 1],
        x_mean,
        x_peak,
...
required.add_argument(
    "-volumetric_repr",
    type=int,
    help="Provide the volumetric_repr argument 1 if volumetric repr, 0 only surface",
    nargs="?",
    default=0,
    required=True,
)
required.add_argument(
    "-Nslice",
    type=int,
    help="Provide the Nslice argument",
    nargs="?",
    default=6,
)
...
if vtk_from_pvpython:
    if infn[-5::] == ".pvtu":
        ...
        infn = []
        for i in numbers:
            infn.append(f"{path}/{file}{i:04d}.pvtu")
        print(infn)
        fn = path + "/" + infn
...
s = f"Coarse: len_min: {len_min} len_max: {len_max} len: {length} len_bub: {len_bub} "
print(s)
...
length = len_max - len_min
s = f"Refined: len_min: {len_min} len_max: {len_max} len: {length} len_bub: {len_bub} "
print(s)
...
s = f"refined_x_mean: {x_mean} refined_u_mean: {u_mean} volume: {volume}"
print(s)
...
x, y = passArray(
    extractSurface1,
    fn=f"r_over_x_total_t={timesteps[0]}.csv",
    PointDataArrays=["Points", "Result"],
...
fn = f"r_over_x_t={timesteps[0]}.csv"
...
plot_graph(
    [XX, [x_peak, x_peak], [x_mean, x_mean]],
    [YY, [0, 0.5], [0, 0.5]],
    ["smoothed", "first peak", "center of mass", "min edge", "max edge"],
    list_x_fill=[lower_hull[:, 0], upper_hull[:, 0]],
    list_y_fill=[lower_hull[:, 1], upper_hull[:, 1]],
    dash=["solid", "dot", "dot"],
    xtitle="x",
    ytitle="r",
    image_name=fn[:-3] + "pdf",
...
for n in range(Nslice):
    ...
    fn = f"slice_t={timesteps[0]}_nalpha_{n}.csv"
    x, y = passArray(clip2, fn, PointDataArrays=["Points", "Result"], CellDataArrays=["Volume"])

    x_peak_, y_peak_, length_x_peak_mean_ = find_first_peak(
        x, y, xy0[0], xmin, xmax, x_mean, timesteps[0],
    )
    print(
        f"slice n={n} x_peak_= {x_peak_} y_peak_= {y_peak_} length_x_peak_mean_= {length_x_peak_mean_}",
    )
    XX, YY, length_x_ends, xy0, xyN, xmin, xmax = sort_and_smooth(
        x,
        y,
        x_mean,
        x_peak_,
    ...
    if num == 0:
        ...
        slices_name.append(f"\u03B1=\u03C0/{den}")
    else:
        slices_name.append(f"\u03B1={num}\u03C0/{den}")
    slices_mode.append("lines")
    slice_dash.append("solid")
    SaveData(
        path + f"/slice_t={timesteps[0]}_nalpha_{n}.csv",
        proxy=clip2,
        CellDataArrays=["vtkGhostType"],
        FieldDataArrays=["TimeValue"],
        AddTime=1,
    )
    fn = f"sliceList_t={timesteps[0]}_nalpha_{n}.csv"
...
fn = f"slice_t={timesteps[0]}.pdf"
# mode=slices_mode, \ #xrange=[xy0[0] - 0.1, xyN[0] + 0.1], \ colors=['blue', 'red', 'green', 'black', 'aqua', 'indigo', 'pink', 'yellow' ]
plot_graph(
    slices_x,
    slices_y,
    slices_name,  # mode =['markers', 'markers', 'markers', 'markers', 'markers'], \
    mode=slices_mode,  # list_x_fill=[upper_hull[:,0], lower_hull[:,0]], list_y_fill=[upper_hull[:,1], lower_hull[:,1]], \
    dash=slice_dash,
    xtitle="x",
    ytitle="r",
    image_name=fn,
    yrange=[0.0, 0.5],
    colors=[],  # colors=['blue', 'red', 'green', 'black', 'blue', 'red', 'green', 'black' ], \
    marker_size=0.5,
    width=1000,
    height=500,
    path="./",
...
with open(fn, "a") as f:
    f.write(
        f"{timesteps[0]} {delta_min} {delta_mean} {delta_max} {delta_min_smooth} {delta_max_smooth} {xy0[0]} {xy0[1]} {xyN[0]} {xyN[1]}\n",
    )
print("Successfully save file:", fn)
