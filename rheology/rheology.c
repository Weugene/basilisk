...
double no_bubbles_only_front (double x, double y, double z)
{
...
double simple_geometry(double x, double y, double z) {
    return 0.01 - sq(x-0.5) - sq(y);
}
/**
## Main function
...
//        fclose (fp);
        ...
        do {
            iteration++;
            fraction(fiber, simple_geometry (x, y, z));
//            fraction(fiber, geometry (x, y, z));
            fraction(f0, no_bubbles_only_front (x, y, z));
...
event vtk_file (t+=0.05; t<10)
{
    ...
    output_vtu_bin_foreach((scalar *) {l, f, fiber, p}, (vector *) {u, muv}, 64, fp, false);
    fclose(fp);
    @if _MPI
    if (pid() == 0) {
        ...
        fp = fopen(name, "w");
        output_pvtu_bin((scalar *) {l, f, fiber, p}, (vector *) {u, muv}, 64, fp, subname);
        fclose(fp);
    }
...
//    output_vtk ({l, f, fiber, u.x, u.y}, 256, fplist, false );
////    output_vtk ({l, f, fiber, u, muv}, 64, fplist, false );
//    iteration++;
//}
